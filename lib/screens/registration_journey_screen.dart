...
// ─────────────────────────────────────────────────────────────────────────────
// RegistrationJourneyScreen — Parcours d'inscription multi-étapes
//
// Reçoit un UserProfile avec profileType déjà renseigné.
// Gère 3 parcours : Urban (7 étapes), Kids (9 étapes), Pro (6 étapes).
// Animation slide horizontal entre étapes.
// Soumission Firebase sur l'avant-dernière étape (RGPD).
// ─────────────────────────────────────────────────────────────────────────────

class RegistrationJourneyScreen extends StatefulWidget {
  const RegistrationJourneyScreen({super.key, required this.initialProfile});

  final UserProfile initialProfile;

  @override
  State<RegistrationJourneyScreen> createState() =>
      _RegistrationJourneyScreenState();
}

class _RegistrationJourneyScreenState extends State<RegistrationJourneyScreen>
  ...
  // Étape RGPD (avant-dernière = où on soumet Firebase)
  int get _rgpdStep {
    return switch (_type) {
      ProfileType.urban => 5,
      ProfileType.kids => 7,
  ...
  Future<void> _nextStep() async {
    ...
    if (!_isTransitionStep) {
      ...
      // Vérification spécifique selon type + étape
      if (_type == ProfileType.urban) {
        if (_stepIndex == 3 && _gender == null) {
          _showSnack('Veuillez sélectionner votre genre.');
          return;
        }
      } else if (_type == ProfileType.kids) {
        if (_stepIndex == 5 && _gender == null) {
          _showSnack('Veuillez sélectionner le genre de votre enfant.');
          return;
        }
        if (_stepIndex == 2) {
          // Gestion spéciale PIN
          if (!_pinConfirmMode) {
            if (_pinCursor < 4) {
            ...
            if (p1 != p2) {
              setState(() {
                ...
                _pinConfirmCursor = 0;
                _pinError = 'Les PINs ne correspondent pas. Réessayez.';
              });
        ...
        if (_stepIndex == 3 && _gender == null) {
          // Pro n'a pas d'étape genre, ignorer
        }
      }
    }

    // Soumission Firebase sur l'étape RGPD
    ...
    // Déclencher animation de succès sur le dernier écran
    if (_isLastStep) {
  ...
  Future<bool> _submitFirebase(AuthProvider auth) async {
    auth.clearError();
  ...
  Widget _buildTopBar(bool isDark) {
    // Étapes non-transition : progression 1-based, en excluant les étapes transition
    final nonTransitionTotal = _totalSteps - 2; // exclure step 0 et dernière
    final nonTransitionCurrent = _stepIndex; // 1 = première non-transition
            ...
            children: [
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(
                      'Étape $nonTransitionCurrent/$nonTransitionTotal',
                      style: TextStyle(
                        fontSize: 11,
  ...
  Widget _buildUrbanStep(bool isDark) {
    return switch (_stepIndex) {
      0 => _buildTransitionWelcome(
          ...
          title: 'Sautez le pas',
          subtitle: 'Créez votre espace personnel en quelques étapes.',
  ...
  Widget _buildKidsStep(bool isDark) {
    return switch (_stepIndex) {
      0 => _buildTransitionWelcome(
          ...
          title: 'Espace Parent',
          subtitle:
              'Suivez le développement de la marche de votre enfant.',
          buttonLabel: 'Commencer',
          onButton: _nextStep,
        ),
          ...
          Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 12),
                Text(
                  'Accès sécurisé avec votre code cabinet.',
                  textAlign: TextAlign.center,
                  style: tt.bodyMedium?.copyWith(
                    color: isDark
                        ? SmartSoleColors.textSecondaryDark
  ...
  // ── Étape : Identifiants (email + mot de passe) ───────────────────────────
      ...
      child: Column(
        ...
        children: [
          _StepHeader(
            ...
            subtitle: 'Utilisé pour calibrer les normes biomécanique IMM.',
          ),
          const SizedBox(height: 32),
          Row(
            children: [
      ...
      child: Column(
        ...
        children: [
          _StepHeader(
            ...
            subtitle: 'Pour calibrer votre analyse biomécanique.',
            iconWidget: Container(
              width: 48,
              height: 48,
              decoration: BoxDecoration(
      ...
      child: Column(
        ...
        children: [
          _StepHeader(
            ...
            subtitle: 'Conformité RGPD — vous gardez le contrôle.',
            iconWidget: Container(
              width: 52,
              height: 52,
              decoration: BoxDecoration(
          ...
          _ConsentTile(
            ...
            subtitle:
                'Sauvegardez vos sessions et accédez-y sur tous vos appareils.',
            value: _consentCloud,
          ...
          _ConsentTile(
            ...
            subtitle:
                'Contribuez à l\'amélioration de SmartSole (données anonymes).',
          ...
          Consumer<AuthProvider>(
            builder: (_, auth, __) => _GradientButton(
              label: auth.isLoading ? '' : 'Continuer',
              isLoading: auth.isLoading,
              onTap: auth.isLoading ? null : _nextStep,
            ),
          ),
        ],
      ),
      ...
      child: Column(
        ...
        children: [
          _StepHeader(
            ...
            subtitle: 'Requis pour accéder à l\'espace professionnel.',
            iconWidget: Container(
              width: 52,
              height: 52,
              decoration: BoxDecoration(
          ...
          if (!_isCabinetCodeValid)
            Consumer<AuthProvider>(
              builder: (_, auth, __) => _GradientButton(
                label: _isVerifyingCode ? '' : 'Vérifier',
                isLoading: _isVerifyingCode,
                onTap: _isVerifyingCode ? null : () => _verifyCabinetCode(auth),
  ...
  Future<void> _verifyCabinetCode(AuthProvider auth) async {
    final key = _formKeys[_stepIndex];
