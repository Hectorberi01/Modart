...
class _LiveDashboardScreenState extends ConsumerState<LiveDashboardScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final leftPressure = _isSessionActive
        ? PressureData(
        ...
        : PressureData.zero;
    final rightPressure = _isSessionActive
        ? PressureData(
    ...
    final mlpi = _isSessionActive ? (session.postureScore - 50) / 100 : 0.0;
    final segment = _isSessionActive ? _computeSegment(session.cadence) : WalkSegment.stopped;
    final speed = _isSessionActive ? (session.distance / 1000) / (_sessionDuration.inSeconds / 3600).clamp(0.001, double.infinity) : 0.0;
                        ...
                        children: [
                          Expanded(
                            child: _KpiCard(
                              ...
                              unit: 'pas/min',
                              icon: Icons.speed,
                              biState: _cadenceBIState(session.cadence),
                              ...
                              child: GlassBentoCard(
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    MLPISlider(value: _isSessionActive ? mlpi : 0),
                                  ],
                                ),
                              ),
                            ),
                      ...
                      GlassBentoCard(
                        ...
                        child: Row(
                          children: [
                            ...
                            Text(
                              _isSessionActive ? '${session.globalScore.toStringAsFixed(0)}%' : '--',
  ...
  Widget _buildTopBar(bool isDark, TextTheme textTheme, int totalSteps, dynamic bluetoothService) {
    return StreamBuilder(
      stream: bluetoothService.adapterState,
      builder: (context, snapshot) {
        final bleOn = snapshot.data == null ? false : snapshot.data.toString().contains('on');
        final isConnected = _isSessionActive; // Simplified: if session active, we're "connected"
  ...
  BIState _cadenceBIState(double cadence) {
